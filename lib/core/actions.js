...
const createPageAction = async (name, dest) => {
  // 1.编译ejs模板
  const pageResult = await compile("component.ejs", { name, lowercaseName: name.toLowerCase() });
  const routeResult = await compile("vue-router.js.ejs", { name, lowercaseName: name.toLowerCase() });
  // -- 生成目标路径 =》写入文件：
  // --- 当未指定路径 =》根据文件名生成同名文件夹：
...
const createStoreAction = async (name, dest) => {
  ...
  const storeResult = await compile("vue-store.ejs", { name, lowercaseName: name.toLowerCase() });
  const storeTypesResult = await compile("vue-store-types.ejs", { name, lowercaseName: name.toLowerCase() });
  const customDir = path.resolve(dest, name.toLowerCase());
  if (generateDir(customDir)) {
    // 2. 分别写入文件：
