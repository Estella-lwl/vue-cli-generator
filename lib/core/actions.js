...
const download = promisify(require("download-git-repo")); // promisify将该库变为可使用Promise
...
const createProjectAction = async (project, others) => {
  const spinner = (await import("ora")).default("Cloning...").start();
  ...
  try {
    ...
    spinner.succeed("Cloning success");
  } catch (error) {
  ...
  try {
    ...
    spinner.fail("Installing failed");
    return;
  }

  // 3.运行 npm run serve，参数和上一步同理
  commandSpawn(sysCommand, ["run", "dev"], { cwd: `./${project}` });
  ...
  open("http://localhost:9528/"); //指定打开的地址
};
