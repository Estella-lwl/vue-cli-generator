...
const createCommands = () => {
  ...
  program
    .command("addcpn <name>")
    .description("add vue project component, e.g. st addcpn HelloWorld [-d src/components]")
    .action((name) => {
  ...
  program
    ...
    .description("add vue page, e.g. st addpage homePage [-d src/views/list/index]")
    .action((page) => {
      createPageAction(page, program.opts().dest || "src/views");
    });
  ...
  program
    ...
    .description("add vue store, e.g. st addstore homeStore [-d src/store/cate]")
    .action((page) => {
